...
function AppContent() {
  ...
  }, [view.name, view.productId, view.category, view.adminTab]);
    ...
    <div className="min-h-screen flex flex-col font-sans selection:bg-black selection:text-white">
      ...
      <main className="flex-grow">
        <AnimatePresence mode="wait">
          ...
          {view.name === 'wishlist' && <Wishlist key="wishlist" />}
        </AnimatePresence>
      </main>
      ...
      {view.name !== 'checkout' && view.name !== 'admin-login' && view.name !== 'admin-dashboard' && (
        <div className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/90 backdrop-blur-md border-t border-gray-200 flex justify-around items-center h-16 pb-safe">
          <button onClick={() => { setView({ name: 'home' }); window.scrollTo(0, 0); }} className="flex flex-col items-center justify-center w-full h-full text-gray-500 hover:text-black transition-colors">
          ...
          <button onClick={() => { setView({ name: 'wishlist' }); window.scrollTo(0, 0); }} className="flex flex-col items-center justify-center w-full h-full text-gray-500 hover:text-black transition-colors">
